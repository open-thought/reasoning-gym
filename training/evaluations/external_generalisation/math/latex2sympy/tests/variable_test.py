...
def test_variable_greek_letter():
    assert_equal(
        ...
        Symbol("\\alpha " + hashlib.md5("\\alpha ".encode()).hexdigest(), real=True) * Symbol("alpha", real=True),
    )
...
def test_variable_greek_letter_subscript():
    assert_equal(
        ...
        Symbol("\\alpha _{\\beta }" + hashlib.md5("\\alpha _{\\beta }".encode()).hexdigest(), real=True)
...
def test_variable_braced_subscript():
    assert_equal(
        ...
        Symbol("x\\beta 10_{y\\alpha 20}" + hashlib.md5("x\\beta 10_{y\\alpha 20}".encode()).hexdigest(), real=True),
    )
...
def test_variable_percentage():
    assert_equal(
        "\\variable{x}\\%",
        Mul(
            Symbol("x" + hashlib.md5("x".encode()).hexdigest(), real=True), Pow(100, -1, evaluate=False), evaluate=False
        ),
    )
