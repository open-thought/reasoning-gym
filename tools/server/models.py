...
class ExperimentCreate(BaseModel):
    ...
    datasets: Dict[str, Dict[str, Any]] = Field(
        ..., 
        description="Dictionary of datasets configurations"
    )
...
class ExperimentResponse(BaseModel):
    ...
    datasets: Dict[str, Dict[str, Any]] = Field(
        ..., 
        description="Current dataset configurations"
    )
...
class ExperimentList(BaseModel):
    ...
    experiments: list[str] = Field(
        ...
        description="List of registered experiment names"
    )
...
class BatchRequest(BaseModel):
    ...
    base_index: int = Field(
        0, 
        description="Starting index for the batch",
        ge=0
    )
...
class CompletionScore(BaseModel):
    ...
    completions: list[Dict[str, Any]] = Field(
        ..., 
        description="List of completion results to score"
    )
